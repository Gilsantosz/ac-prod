import { buildDashboardGoalAnalysis, aggregateGoalBuckets, goalStatusLabel } from '@/lib/dashboardGoalAnalysis';
...
export function createProductionAnalysisReport(snapshot, { generatedBy = '' } = {}) {
  ...
  const analysis = snapshot.goalContext ? buildDashboardGoalAnalysis(snapshot.entries || [], snapshot.goalContext.goals || [], { ...snapshot.goalContext, period: snapshot.period, filters: snapshot.filters }) : buildOperationalAnalysis(snapshot.entries || [], snapshot.comparisonPeriod ? snapshot.comparisonEntries || [] : []);
  ...
  units.forEach((unit) => {
    ...
    for (const [metric, label, format, mode, lowerIsBetter] of [
      ...
      summary.push({ key, label: `${label} · ${unit.unitLabel}`, value: unit[metric] == null || (metric === 'produced' && unit.measurementPending && !unit.count) ? null : unit[metric] / divisor,
  ...
  const monthlyRows = units.flatMap((unit) => (analysis.goalBuckets ? aggregateGoalBuckets(analysis.goalBuckets.filter((e) => e.metric_unit === unit.key), (e) => e.date.slice(0, 7)) : aggregateAnalysis(entries.filter((e) => e.metric_unit === unit.key), (e) => e.date.slice(0, 7)))
  ...
  const charts = units.map((unit) => {
    ...
    return { id: `volume-${unit.key}`, title: `Produzido × meta · ${unit.unitLabel}`, type: 'bar', unit: ` ${unit.unitLabel}`,
      categories: rows.map((r) => r.label), series: [
        { name: 'Produzido', color: '#15803d', values: rows.map((r) => r.measurementPending && !r.count ? null : r.produced) },
    ...
    tables: [
      { id: 'production-data', title: 'Base detalhada de produção válida', sheet: 'data', primary: true, rows: dataRows,
        columns: [
          ...
          ...METRIC_COLUMNS.map((c) => ({ ...c, ...(analysis.goalBuckets && c.key === 'target' ? { label: 'Meta do lançamento (histórica)' } : {}), pdf: ['unitLabel', 'produced', 'target', 'attainment', 'scrap', 'downtime'].includes(c.key) })),
      ...
      { id: 'production-by-cell', title: 'Diagnóstico por célula e unidade', sheet: 'analysis',
        columns: [{ key: 'cell', label: 'Célula', type: 'text', width: 24 }, ...METRIC_COLUMNS], rows: cells.map((c) => fractions({ ...c, produced: c.measurementPending && !c.count ? null : c.produced })) },
      ...
      ...(analysis.goalBuckets ? [{ id: 'goal-reconciliation', title: 'Conferência de metas vigentes por dia, célula e turno', sheet: 'analysis',
        columns: [{ key: 'date', label: 'Data', type: 'date' }, { key: 'cell', label: 'Célula', type: 'text', width: 22 },
          { key: 'shift', label: 'Turno', type: 'text', width: 16 }, { key: 'unitLabel', label: 'Unidade', type: 'text' },
          { key: 'produced', label: 'Produção medida', type: 'number' }, { key: 'target', label: 'Meta vigente', type: 'number' },
          { key: 'goal_date', label: 'Origem da meta', type: 'date' }, { key: 'goal_status_label', label: 'Vínculo', type: 'text', width: 24 }],
        rows: analysis.goalBuckets.map((b) => ({ ...b, produced: b.count ? b.produced : null, goal_status_label: goalStatusLabel(b.status) })) }] : []),
    ],
    metadata: { source: analysis.goalBuckets ? 'production_entries + production_daily_goals + workday_calendar' : 'production_entries', generatedAt: snapshot.generatedAt, fetchedRowCount: snapshot.fetchedRowCount,
