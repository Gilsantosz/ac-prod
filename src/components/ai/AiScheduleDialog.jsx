...
export default function AiScheduleDialog({ open, onOpenChange, report, recipients = [], onSave }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle className="flex gap-2 items-center"><CalendarClock className="w-5 h-5" />Agendar relatório</DialogTitle><DialogDescription>O processamento ocorrerá no servidor e usará Usuários/Gestores como fonte oficial dos destinatários.</DialogDescription></DialogHeader><div className="grid grid-cols-2 gap-4"><div className="space-y-2"><Label>Frequência</Label><Select value={frequency} onValueChange={setFrequency}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="daily">Diário</SelectItem><SelectItem value="workdays">Dias úteis</SelectItem><SelectItem value="weekly">Semanal</SelectItem><SelectItem value="monthly">Mensal</SelectItem></SelectContent></Select></div><div className="space-y-2"><Label>Horário</Label><Input type="time" value={timeLocal} onChange={(event) => setTimeLocal(event.target.value)} /></div><div className="space-y-2 col-span-2"><Label>Modelo de e-mail</Label><AiTemplateSelector value={templateCode} onChange={setTemplateCode} /></div><div className="space-y-2 col-span-2"><Label>Destinatários</Label><div className="max-h-36 overflow-y-auto border border-border rounded-md divide-y divide-border">{recipients.length ? recipients.map((item) => <label key={item.id} className="flex items-center gap-2 p-2 text-sm"><Checkbox checked={recipientIds.includes(item.id)} onCheckedChange={() => toggle(item.id)} />{item.name} · {item.email}</label>) : <p className="p-2 text-sm text-muted-foreground">Cadastre um gestor em Usuários → Gestores.</p>}</div></div></div><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancelar</Button><Button disabled={!recipientIds.length} onClick={() => onSave({ name: report?.title, reportType: report?.reportType, format: report?.format, filters: report?.filters, options: report?.options, frequency, timeLocal, recipientIds, templateCode })}>Agendar</Button></DialogFooter></DialogContent></Dialog>;
}
