#!/usr/bin/env python3
"""Fail-closed static audit for the AC.Prod2 collection rollout."""
...
def main() -> int:
    ...
    require_all(history_sql, ("ADD COLUMN IF NOT EXISTS raw_value", "GENERATED ALWAYS AS (tag_value) STORED", "ADD COLUMN IF NOT EXISTS traceability_code", "idx_stage_readings_traceability_code_compat"), "compatibilidade do Histórico")
    final_sql = read(migrations / "20260831052721_finalize_collection_rollout_v6.sql")
    require_all(final_sql, ("process_production_reading_v2(jsonb)", "resolve_operator_shift_window(uuid,timestamptz)", "recalculate_cell_lot_state(uuid,text,text,uuid,uuid)", "refresh_collection_lot_state(uuid,uuid)", "production_cell_lot_states", "production_cell_active_contexts", "ACPROD_COLLECTION_V6_DUPLICATES", "ACPROD_COLLECTION_V6_LOT_DRIFT"), "contrato SQL final")
    probe_sql = read(migrations / "20260831052809_publish_collection_release_probe_v7.sql")
    require_all(probe_sql, ("get_public_collection_release", "publish_collection_release_probe_v7", "20260831_acprod_collection_db_v7", "history_compatibility", "GRANT EXECUTE ON FUNCTION public.get_public_collection_release()", "TO anon,authenticated"), "marcador público")

    combined_sql = "\n".join(read(path) for path in migrations.glob("2026083105*.sql"))
    ...
    require_all(workflow, ("pull_request:", "REQUIRED_MIGRATION_VERSION: \"20260831052809\"", "REQUIRED_RELEASE_VERSION: \"20260831_acprod_collection_db_v7\"", "get_public_collection_release", "DATABASE_RELEASE_OK", "needs: [database-release]", "actions/checkout@v6", "actions/setup-node@v6", "actions/upload-pages-artifact@v5", "actions/deploy-pages@v5"), "workflow")
    ...
    require_all(collection_service, ("get_collection_dashboard_snapshot_v2", "get_operator_shift_kpis_v2"), "serviço de coleta")
    page = read(repo / "src" / "pages" / "TraceabilityCollection.jsx")
    if "modalOpen={isAnyModalOpen}" not in page:
        fail("página não suspende scanner durante modais")
    item = read(repo / "src" / "components" / "collection" / "CollectionReadItem.jsx")
    action_start = item.find("{/* Ações rápidas no card */}")
    if action_start < 0:
        fail("bloco de ações do Histórico não localizado")
    action_block = item[action_start:]
    ...
    print("AUDIT_COLLECTION_ROLLOUT_OK")
    print(f"ledger_versions={len(REQUIRED_LEDGER)}")
    print("database_gate=public_release_probe_fail_closed")
    print("history_modal=protected")
    print("realtime_lifecycle=enabled")
    return 0
