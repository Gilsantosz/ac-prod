...
export default function TraceabilityKpiCards({ kpis = {} }) {
  ...
  const producedInShift = Number(kpis.produced_this_shift ?? kpis.total) || 0;
  const items = [
    {
      key: 'produced_this_shift',
      label: unit ? `Produzido no turno (${unit})` : 'Produzido no turno',
      value: producedInShift,
      icon: ScanLine,
      color: 'text-sky-600 bg-sky-50 dark:bg-sky-950/25',
    },
      ...
      {items.map(({ key, label, value, icon: Icon, color }) => (
        <div key={key} className="bg-card border border-border rounded-md p-4 flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-2xl font-bold text-foreground tabular-nums">{value ?? kpis[key] ?? 0}</p>
          </div>
        </div>
      ))}
    </div>
