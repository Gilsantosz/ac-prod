...
async function serveStaticBuild(page) {
  ...
  await page.route(/^https?:\/\/app[.]test\//, async (route) => {
    ...
    try {
      ...
      await route.fulfill({ status: 200, contentType: MIME_TYPES[path.extname(filePath)] || 'application/octet-stream', body });
...
function jwt() {
  ...
  return `${encode({ alg: 'HS256', typ: 'JWT' })}.${encode({
    sub: userId, email: 'operador.teste@leo.com.br', role: 'authenticated',
...
async function installBrowserObservers(page) {
  await page.addInitScript(() => {
    ...
    class FakeAudioContext {
      ...
      createOscillator() {
        return { connect() {}, frequency: { setValueAtTime() {} }, onended: null,
          start() { window.__collectionApprovalAudioStarts += 1; }, stop() { this.onended?.(); } };
      }
    ...
    Object.defineProperty(navigator, 'vibrate', {
      configurable: true,
      value: () => { window.__collectionVibrations += 1; return true; },
    });
    // Deterministic fixture only: retain the previous reconciliation timing.
    const nativeSetInterval = window.setInterval.bind(window);
    window.setInterval = (callback, timeout, ...args) => nativeSetInterval(callback, timeout === 15_000 ? 200 : timeout, ...args);
  });
}
async function mockCollectionV3(page) {
  const state = { finalize: false, ingestedEnvelope: null, ingestedDeviceId: null, clientEventIds: [], contextConfirmations: 0 };
  const user = {
    id: userId, email: 'operador.teste@leo.com.br', aud: 'authenticated', role: 'authenticated',
    user_metadata: { name: 'Operador Teste', role: 'operator', cell: 'Corte' },
    app_metadata: { provider: 'email' }, created_at: receivedAtDb,
  };
  const profile = {
    id: userId, email: user.email, name: 'Operador Teste', role: 'operator', cell: 'Corte',
    permissions: { view_dashboards: true, register_production: true, manage_occurrences: true,
      view_reports: true, view_traceability: true, view_replacements: true, manage_replacements: true },
  };
  await page.route('**://*.supabase.co/**', async (route) => {
    ...
    if (requestPath.endsWith('/auth/v1/token')) {
      return fulfill({ access_token: jwt(), token_type: 'bearer', expires_in: 3600,
        expires_at: Math.floor(Date.now() / 1000) + 3600, refresh_token: 'refresh-token-test', user });
    }
    ...
    if (requestPath.endsWith('/rest/v1/cells')) {
      return fulfill([{ id: 'cell-cut', name: 'Corte', active: true,
        shift_hours: { shift1: 8, shift2: 8, shift3: 8 }, notes: '' }]);
    }
    if (requestPath.endsWith('/rest/v1/rpc/operator_login_v2')) {
      return fulfill({ success: true, session_id: 'operator-session-test', session_token: 'operator-token-v2-only',
        expires_at: new Date(Date.now() + 8 * 60 * 60 * 1000).toISOString(),
        operator: { id: 'operator-test', name: 'Operador Teste', login_name: 'operador.teste',
          registration_masked: '***123', shift: '1º Turno', primary_cell_id: 'cell-cut', primary_machine_id: null,
          cells: [{ id: 'cell-cut', name: 'Corte', is_primary: true }], machines: [] },
      });
    }
    if (requestPath.endsWith('/rest/v1/rpc/set_operator_session_context')) {
      const payload = request.postDataJSON();
      const authorized = payload.p_session_token === 'operator-token-v2-only'
        && payload.p_cell_id === 'cell-cut' && payload.p_machine_id === null
        && payload.p_station_name === 'Coletor Chão de Fábrica';
      if (!authorized) return fulfill({ success: false, error: 'TEST_CONTEXT_NOT_AUTHORIZED' }, 403);
      state.contextConfirmations += 1;
      return fulfill({ success: true, cell_name: 'Corte', machine_name: null });
    }
    if (requestPath.endsWith('/rest/v1/rpc/logout_operator_session') || requestPath.endsWith('/rest/v1/rpc/heartbeat_operator_session')) {
    ...
    if (requestPath.endsWith('/rest/v1/rpc/get_collection_pipeline_flags_v3')) {
      return fulfill({ collection_pipeline_v3_ingress: { enabled: true, rollout_scope: { all: true } },
    ...
    if (requestPath.endsWith('/rest/v1/rpc/ingest_collection_batch_v3')) {
      ...
      return fulfill({ batch_id: payload.p_batch_id, device_id: payload.p_device_id, received_at_db: receivedAtDb,
        results: state.clientEventIds.map((clientEventId) => ({ client_event_id: clientEventId, persisted: true, received_at_db: receivedAtDb })) });
    }
    if (requestPath.endsWith('/rest/v1/coletas_producao')) {
      if (!state.finalize) return fulfill([]);
      return fulfill(state.clientEventIds.map((clientEventId) => ({ client_event_id: clientEventId, status_sincronizacao: 'sincronizada',
        resultado: { client_event_id: clientEventId, collection_state: 'APPROVED', status: 'approved', success: true, message: 'Leitura aprovada pelo pipeline V3.' },
        erro: null, retryable: false, batch_id: 'batch-v3-e2e', received_at_db: receivedAtDb,
        server_received_at: receivedAtDb, processado_em: new Date().toISOString(), last_error_code: null })));
    }
...
test('offline e ACK permanecem neutros até APPROVED após a reconexão', async ({ page, context }) => {
  ...
  await expect.poll(() => state.contextConfirmations).toBeGreaterThan(0);
  ...
  await expect(feedback).toHaveAttribute('data-collection-state', 'DATABASE_ACKNOWLEDGED', { timeout: 10_000 });
  await expect(feedback).not.toHaveClass(/border-emerald-300/);
  await expect(page.getByText('PEÇA LIBERADA — OK')).toHaveCount(0);
  await expect.poll(() => page.evaluate(() => window.__collectionApprovalAudioStarts)).toBe(0);
  await expect.poll(() => page.evaluate(() => window.__collectionVibrations)).toBe(0);
  expect(state.ingestedEnvelope).toMatchObject({ operator_session_id: 'operator-session-test', source_mode: 'offline_replay',
    events: [{ raw_value: '09950001', tag_lida: '09950001', reader_type: 'keyboard_barcode', device_sequence: expect.any(Number), quantity: 1 }] });
  expect(state.ingestedEnvelope.events).toHaveLength(1);
  expect(state.ingestedDeviceId).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i);
  expect(JSON.stringify(state.ingestedEnvelope)).not.toMatch(/token|jwt|authorization/i);
  state.finalize = true;
  await expect(feedback).toHaveAttribute('data-collection-state', 'APPROVED', { timeout: 10_000 });
