...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!Array.isArray(body.recipientIds) || !body.recipientIds.length) throw new Error('RECIPIENT_REQUIRED');
    ...
    if (recipientsError || !recipients?.length) throw new Error('RECIPIENTS_NOT_FOUND');
    ...
    for (const recipient of recipients) {
      ...
      await admin.from('report_email_logs').insert({ report_job_id: job.id, recipient_id: recipient.id, recipient_email: recipient.email, subject, provider: 'resend', provider_message_id: providerResult.id || null, status: success ? 'sent' : 'failed', error_message: errorMessage, sent_by: user.id, sent_at: success ? new Date().toISOString() : null });
      results.push({ email: recipient.email, success, error: errorMessage });
    }
    const sent = results.filter((item) => item.success).length;
    const failed = results.length - sent;
    await recordSystemLog(admin, { user_id: user.id, trace_id: job.trace_id, level: sent ? 'info' : 'error', event: sent ? 'report.email.sent' : 'report.email.failed', entity: 'report_job', entity_id: job.id, metadata: { recipients: results.length, success: sent, failed }, success: sent > 0 });
