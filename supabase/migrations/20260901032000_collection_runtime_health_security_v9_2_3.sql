-- AC.Prod2 v9.2.3 — endurecimento fail-closed das policies e do segredo do worker.
--
-- A v9.2.2 já pode estar registrada em produção. Esta migração incremental e
-- idempotente redefine somente o probe autoritativo, preserva os wrappers
-- legados e registra um novo snapshot histórico para o gate v9.2.3.
  ...
  async_flags AS (
    SELECT jsonb_build_object(
      ...
      'collection_runtime_worker_secret_verifier',
        ...
        AND definitions.verify_secret_source =
          'selectexists(select1fromvault.decrypted_secretssecret'
          || 'wheresecret.name=''acprod_collection_worker_secret'''
          || 'andextensions.digest(secret.decrypted_secret,''sha256'')='
...
SELECT
  '20260901_acprod_collection_runtime_health_security_v9_2_3',
  'dynamic-runtime-catalog-health-rls-secret-verifier-v9-2-3',
  'Probe fail-closed v9.2.3: policies RLS validadas por comando, roles e expressões efetivas; verificador do segredo validado pelo corpo canônico com Vault e SHA-256.',
  'v9.2.3',
  (value ->> 'ready')::boolean,
  value -> 'schema_flags'
FROM runtime_health
