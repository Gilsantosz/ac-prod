...
describe('AC.Prod2 collection micro-batching v8.6 contract', () => {
  ...
  it('impede deploy quando a saúde dinâmica da coleta não estiver na v9.2.3', () => {
    ...
    expect(workflow).toContain("payload.get('health_source') != 'runtime_catalog_snapshot'");
    expect(workflow).toContain("payload.get('snapshot_used') is not True");
    expect(workflow).toContain("payload.get('snapshot_format') != 'collection_runtime_health_snapshot_v1'");
    expect(workflow).toContain("payload.get('snapshot_status') != 'fresh'");
    ...
    expect(workflow).toContain('"collection_health_probe": "runtime_catalog_snapshot"');
  });
});
