...
export default function OperationalAlertsPanel() {
  ...
  const [filterGeneralLot, setFilterGeneralLot] = useState('all');
  ...
  const generalLots = useMemo(() => {
    const set = new Set(alerts.map(a => a.metadata?.general_lot_code).filter(Boolean));
    return ['all', ...Array.from(set)];
  }, [alerts]);
  ...
  const filteredAlerts = useMemo(() => {
    return alerts.filter(a => {
      ...
      const matchesGeneralLot = filterGeneralLot === 'all' || String(a.metadata?.general_lot_code || '') === String(filterGeneralLot);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Painel de Filtros */}
      <div className="bg-slate-50 dark:bg-slate-900/30 p-4 border border-border/50 rounded-2xl grid grid-cols-2 md:grid-cols-5 gap-3">
        <div>
        ...
        <div>
          <label htmlFor="filter-general-lot" className="block text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-1.5">
            Lote Geral
          </label>
          <select
            id="filter-general-lot"
            value={filterGeneralLot}
            onChange={(e) => setFilterGeneralLot(e.target.value)}
            className="w-full bg-background border border-border/80 rounded-xl px-2.5 py-1.5 text-xs focus:ring-1 focus:ring-rose-500 focus:outline-none font-mono"
          >
            <option value="all">Todos Os Lotes Gerais</option>
            {generalLots.filter(l => l !== 'all').map(l => (
              <option key={l} value={l}>Lote Geral: {l}</option>
            ))}
          </select>
        </div>
                    ...
                    <div className="flex items-center justify-between gap-3 pt-2.5">
                      ...
                      <div className="flex gap-2 shrink-0 flex-wrap">
                        <Button
                          size="sm"
                          variant="ghost"
                          className="h-7 text-[10px] text-blue-600 hover:text-blue-700 hover:bg-blue-50/50 rounded-lg px-2.5"
                        ...
                        {!isResolved && (
                          <Button
                            size="sm"
                            variant="ghost"
                            className="h-7 text-[10px] text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50/50 rounded-lg px-2.5 border border-emerald-100 hover:border-emerald-200 font-semibold"
