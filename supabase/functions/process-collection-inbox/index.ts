...
async function authorize(req: Request): Promise<{ ok: boolean; source: string }> {
  ...
  if (cronSecret) {
    ...
    if (!error && data === true) {
      return { ok: true, source: "database-wakeup" };
    }
  }
  ...
  if (token) {
    ...
    if (!error && data.user) {
      return { ok: true, source: "authenticated-client" };
    }
  }
...
Deno.serve(async (req: Request) => {
  ...
  const concurrency = clampInteger(body.concurrency, 8, 1, 12);
  const maxRounds = clampInteger(body.max_rounds, 3, 1, 5);
