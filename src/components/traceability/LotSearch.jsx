...
import { KANBAN_STAGES, translateStage } from '@/hooks/useTraceability';
...
function SearchResult({ lot }) {
  ...
  const batch = lot.pcp_import_batch || null;
  const total = Number(progress.total || 0);
  const collected = Number(progress.completed || 0);
  const pending = Number(progress.pending || 0);
  const percent = Math.max(0, Math.min(100, Number(progress.percent || 0)));
  const batchPercent = Math.max(0, Math.min(100, Number(batch?.progress_percent || 0)));
    ...
    <div className="bg-card border border-border/60 rounded-xl p-4 space-y-3">
        ...
        <div className="min-w-0 flex-1">
          {batch?.general_lot_code && (
            <p className="text-[10px] uppercase tracking-wide text-muted-foreground">
              Lote geral <strong className="font-mono text-foreground">{batch.general_lot_code}</strong>
            </p>
          )}
      ...
      {batch?.general_lot_code && (
        <div className="rounded-lg border border-border/50 bg-secondary/20 p-2.5 space-y-1">
          <div className="flex justify-between text-[11px] text-muted-foreground gap-2">
            <span className="truncate">Lote geral {batch.general_lot_code}</span>
            <span className="shrink-0">{batchPercent}% · {Number(batch.completed_parts || 0)}/{Number(batch.total_parts || 0)} finalizadas</span>
          </div>
          <div className="h-1.5 bg-secondary rounded-full overflow-hidden">
            <div className="h-full bg-blue-500" style={{ width: `${batchPercent}%` }} />
          </div>
        </div>
      )}
      ...
      <div className="grid sm:grid-cols-2 gap-2 text-xs text-muted-foreground">
        <span className="flex items-center gap-1 min-w-0">
          ...
          <span className="truncate">{translateStage(lot.current_step)}</span>
        </span>
        <span className="flex items-center gap-1 min-w-0">
