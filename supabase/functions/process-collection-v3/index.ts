...
if (!SUPABASE_URL || !SERVICE_ROLE_KEY) {
  throw new Error("COLLECTION_V3_WORKER_ENVIRONMENT_INCOMPLETE");
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const summary = {
      event: "collection_v3_decision_worker_completed",
      invocation_id: invocationId,
      rounds,
    ...
    console.error(JSON.stringify({
      event: "collection_v3_decision_worker_failed",
      invocation_id: invocationId,
      ...
      if (error) {
        console.error(JSON.stringify({
          event: handoffRequired
            ? "collection_v3_decision_handoff_failed"
            : "collection_v3_decision_slot_release_failed",
          invocation_id: invocationId,
