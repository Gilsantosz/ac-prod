...
DECLARE
  ...
  v_rewritten text;
BEGIN
  SELECT pg_get_functiondef('public.process_production_reading_impl_v2(jsonb)'::regprocedure)
  INTO v_definition;
  ...
  IF position(
    ...
    IN pg_get_functiondef('public.process_production_reading_impl_v2(jsonb)'::regprocedure)
  ) > 0 THEN
...
BEGIN
  ...
  WITH candidates AS (
    ...
    WHERE (
      ...
      AND coalesce(inbox.lease_expires_at, '-infinity'::timestamptz) <= clock_timestamp()
    )
...
BEGIN
  ...
  IF v_row.status_sincronizacao = 'processando'
     ...
     AND coalesce(v_row.lease_expires_at, '-infinity'::timestamptz) > clock_timestamp() THEN
  ...
  IF v_secret.coleta_id IS NULL OR v_secret.expires_at <= clock_timestamp() THEN
    ...
    SET status_sincronizacao = 'erro',
        ...
        queue_delay_ms = extract(epoch from (v_started_at - v_row.server_received_at)) * 1000,
        lease_expires_at = NULL,
        worker_id = NULL,
        updated_at = v_finished_at
    WHERE id = v_row.id;
    DELETE FROM private.coleta_producao_credentials WHERE coleta_id = v_row.id;
    RETURN jsonb_build_object('processed', true, 'status', 'error', 'client_event_id', v_row.client_event_id);
  END IF;
  ...
  PERFORM set_config(
    'request.jwt.claims',
    jsonb_build_object('sub', v_row.auth_user_id, 'role', 'authenticated')::text,
    true
  );
  ...
  IF v_sqlstate IS NULL THEN
    ...
    SET status_sincronizacao = 'sincronizada',
        retryable = false,
        resultado = coalesce(v_result, jsonb_build_object(
          'success', false,
          'status', 'error',
  ...
  v_retryable := left(v_sqlstate, 2) = '08'
    OR v_sqlstate IN ('40001', '40P01', '55P03', '57014', '57P01', '53300');
  v_finished_at := clock_timestamp();

  IF v_retryable AND v_row.attempt_count < 12 THEN
    v_delay_seconds := least(60, greatest(1, power(2::numeric, greatest(v_row.attempt_count - 1, 0))));
    UPDATE public.coletas_producao
    SET status_sincronizacao = 'recebida',
        ...
        last_error_at = v_finished_at,
        next_attempt_at = v_finished_at + make_interval(secs => v_delay_seconds::double precision),
        lease_expires_at = NULL,
        worker_id = NULL,
        updated_at = v_finished_at
    WHERE id = v_row.id;
  ...
  SET status_sincronizacao = 'erro',
      retryable = false,
      erro = left(v_message, 1000),
      last_error_code = coalesce(v_sqlstate, 'COLLECTION_WORKER_ERROR'),
      last_error_at = v_finished_at,
      resultado = jsonb_build_object(
        'success', false,
        'status', 'error',
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER PUBLICATION supabase_realtime ADD TABLE public.coletas_producao;
  END IF;
END;
$$;

-- Recuperação automática caso o wake-up imediato ou a Edge Function sofram
-- indisponibilidade temporária. Uma execução a cada 10s é suficiente porque o
-- trigger é o caminho normal e imediato.
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'run-process-collection-inbox') THEN
  ...
  flags AS (
    SELECT jsonb_build_object(
      ...
      'collection_async_private_credentials',
        ...
        AND NOT has_table_privilege('authenticated', 'private.coleta_producao_credentials', 'SELECT')
        AND NOT has_table_privilege('service_role', 'private.coleta_producao_credentials', 'SELECT'),
      'collection_async_worker_rpcs',
        ...
        AND has_function_privilege('service_role', 'public.claim_collection_inbox(text,integer)', 'EXECUTE')
        AND has_function_privilege('service_role', 'public.process_collection_inbox_item(uuid,text)', 'EXECUTE')
        AND NOT has_function_privilege('authenticated', 'public.claim_collection_inbox(text,integer)', 'EXECUTE')
        AND NOT has_function_privilege('authenticated', 'public.process_collection_inbox_item(uuid,text)', 'EXECUTE'),
