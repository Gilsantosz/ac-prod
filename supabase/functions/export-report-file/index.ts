...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (format === 'csv') {
      const content = '\uFEFF' + [['Leo Madeiras','Leo Flow'],['Relatório',job.title],['Produzido',summary.produced],['Aprovado',summary.approved],['Reprovado',summary.rejected],['Pendente',summary.pending],['Meta',summary.target],['Eficiência',summary.efficiency],[],['Pedido','Lote','Carga','Cliente','Razão Social','Produto','Roteiro','Finalização','Pallet','Célula','Etapa','Produzido','Aprovado','Reprovado','Pendente','Refugo','Parada','Status'],...(snapshot.entries || []).map((row: any) => [row.order_number,row.lot_code,row.load_number,row.customer_trade_name || row.customer_name,row.customer_legal_name,row.product_name,row.route_name,row.finalization_date,row.pallet_number,row.cell,row.process_step,row.produced,row.approved_quantity,row.rejected_quantity,row.pending_quantity,row.scrap,row.downtime,row.approval_status])].map((row) => row.map(esc).join(';')).join('\n');
      return json({ success: true, fileName: `${job.title}.csv`, mimeType: 'text/csv', contentBase64: btoa(unescape(encodeURIComponent(content))) });
    }
    const html = `<!doctype html><html><body style="font-family:Arial;background:#f1f5f9;margin:0;padding:24px"><main style="max-width:820px;margin:auto;background:#fff;border-radius:18px;overflow:hidden"><header style="background:#00522d;color:#fff;padding:24px"><b style="color:#fff200;font-size:28px">Leo Madeiras</b><div style="margin-top:4px">Leo Flow - Relatórios Industriais</div></header><section style="padding:28px"><h1 style="font-size:26px;line-height:1.25">${job.title}</h1><table style="width:100%;border-collapse:separate;border-spacing:8px"><tr><td style="background:#f8fafc;padding:16px;border-radius:12px"><small>PRODUZIDO</small><strong style="display:block;font-size:24px;color:#07844b">${summary.produced || 0}</strong></td><td style="background:#f8fafc;padding:16px;border-radius:12px"><small>META</small><strong style="display:block;font-size:24px">${summary.target || 0}</strong></td><td style="background:#f8fafc;padding:16px;border-radius:12px"><small>EFICIÊNCIA</small><strong style="display:block;font-size:24px;color:#2563eb">${Number(summary.efficiency || 0).toFixed(1)}%</strong></td></tr></table></section></main></body></html>`;
